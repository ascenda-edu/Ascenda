 ...
 * a sigmoid-based admission-probability model, cluster-driven field-of-study
 * matching (with a keyword fallback for rows whose field_of_study is blank),
 * multi-currency fee estimation, aggressive course-name dedup, and
 * institution quality gates.
 *
 * NOTE: the Python original also carried a free-text FIELD_SYNONYMS table
 * ("cybersecurity" → "Computer Science & IT", …). It was ported dead in
 * 1dd583b and removed 2026-07-25 — this engine takes a typed
 * `IntendedCluster[]`, never free text, so nothing could ever consume it.
 * Recover it from git history if a free-text field query is ever added.
 *
