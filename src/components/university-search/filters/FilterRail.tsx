...
export function FilterRail({ children, onClearAll, activeFilterCount }: FilterRailProps) {
  ...
  return (
    <aside className="surface-card surface-card--static sticky top-20 sm:top-24 max-h-[calc(100vh-7.5rem)] overflow-y-auto !p-0">
      <div className="flex items-center justify-between gap-3 border-b border-border px-5 py-4 dark:border-white/10">
        ...
        {activeFilterCount > 0 ? (
          <button
            type="button"
            onClick={onClearAll}
            className="rounded-full px-2 py-1 text-sm font-medium text-primary transition-colors cursor-pointer hover:text-primary/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
          >
            Clear all
          </button>
        ) : null}
