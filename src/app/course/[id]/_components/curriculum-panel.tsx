        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {yearSections.map((section, idx) => (
            <section key={idx} className="surface-card p-0">
              <h3 className="flex items-center gap-3 border-b border-border bg-muted px-5 py-4 text-lg font-semibold text-foreground">
                <span
              ...
              <ul className="divide-y divide-border">
                {section.items.map((item, i) => (
                  <li key={i} className="flex items-start gap-3 p-4 transition-colors hover:bg-muted">
                    <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-primary/60" aria-hidden />
                    <span className="text-sm leading-relaxed text-foreground">{emphasize(item)}</span>
                  </li>
                ))}
              </ul>
            </section>
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {(showAllFlat ? moduleItems : moduleItems.slice(0, FLAT_MODULE_PREVIEW)).map((item, idx) => (
            <li key={idx} className="surface-subcard flex items-start gap-3">
              ...
              <span className="text-sm font-medium leading-relaxed text-foreground">{emphasize(item)}</span>
            </li>
          ))}
        </ul>
      ) : (
